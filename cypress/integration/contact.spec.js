...
describe("Contact Page", () => {
  ...
  it("Verify validations mandatory", () => {
    contact.clickSend();
    cy
      .get(data.locatorsContact.nameError)
      .invoke("text")
      .should("eq", data.textsContact.validateName);
    cy
      .get(data.locatorsContact.emailError)
      .invoke("text")
      .should("eq", data.textsContact.validateEmail);
    cy
      .get(data.locatorsContact.commentError)
      .invoke("text")
      .should("eq", data.textsContact.validateComment);
  });
  ...
  it("Verify incorrect email", () => {
    ...
    cy
      .get(data.locatorsContact.emailError)
      .invoke("text")
      .should("eq", data.textsContact.incorrectEmail);
  });
  ...
  it("Verify > 0 guests", () => {
    ...
    cy.get(data.locatorsContact.guests).then($input => {
      expect($input[0].validationMessage).to.eq(
        data.textsContact.validationMessageGuests
      );
    });
  });
  ...
  it("contact is sending error", () => {
    ...
    contact.clickSend();
    cy
      .get(data.locatorsContact.errorMessage)
      .invoke("text")
      .should("eq", "Error");
  });
